...
Our agent now has memory, tools, robustness, and real knowledge to draw on. But that memory from agent-003 has a flaw we've been ignoring: it grows forever. Every turn, every tool result, every retrieved chunk piles onto the `messages` list we resend on every call -- and eventually it won't fit. [Agent-010]({{ site.baseurl }}{% post_url 2026-07-09-agent-010-context-compaction %}) faces that wall and adds the standard fix: when the history gets too long, the agent turns its one skill -- calling the model -- on its *own* memory, summarising the old turns to make room. The memory arc that opened in agent-003 closes there.
