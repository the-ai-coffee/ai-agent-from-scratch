...
We now have the complete heartbeat of an agent: call, act, feed back, answer, repeat. But our agent still has exactly one tool, hardwired into the loop. Real agents carry a toolbox, and the interesting question becomes one of *choice*: given several tools, how does the model pick the right one -- or decide it needs none at all? [Agent-007]({{ site.baseurl }}{% post_url 2026-07-01-agent-007-multi-tool-dispatch %}) gives our agent a second tool and a proper registry to dispatch between them, turning the single hardcoded gadget into a system any tool can plug into. The loop we just built is exactly what makes that possible -- next.
