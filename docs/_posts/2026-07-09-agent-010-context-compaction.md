...
The memory arc that opened in agent-003 closes here: the agent now remembers, uses, and -- when it must -- condenses its own past. But compaction is the cleanup crew: it shrinks the mess *after* it has already piled into the history, and pays for it in lost detail. [Agent-011]({{ site.baseurl }}{% post_url 2026-07-11-agent-011-subagents %}) attacks the same problem from the opposite direction -- and demystifies the loudest buzzword in the room while it's at it: **subagents**. A subagent is just a tool whose implementation is another agent with its own fresh history; the noisy work happens there, and the parent only ever sees the answer.
