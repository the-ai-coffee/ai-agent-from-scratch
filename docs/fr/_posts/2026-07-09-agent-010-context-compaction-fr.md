...
L'arc de la mémoire ouvert dans agent-003 se referme ici : l'agent sait désormais se souvenir, s'en servir, et — quand il le faut — condenser son propre passé. Mais la compaction, c'est l'équipe de nettoyage : elle rétrécit le désordre *après* qu'il s'est déjà entassé dans l'historique, et le paie en détails perdus. [Agent-011]({{ site.baseurl }}{% post_url 2026-07-11-agent-011-subagents-fr %}) attaque le même problème par l'autre bout — et démystifie au passage le mot à la mode le plus bruyant du moment : les **subagents**. Un subagent, c'est juste un outil dont l'implémentation est un autre agent avec son propre historique tout neuf ; le travail bruyant se passe là-bas, et le parent ne voit jamais que la réponse.
