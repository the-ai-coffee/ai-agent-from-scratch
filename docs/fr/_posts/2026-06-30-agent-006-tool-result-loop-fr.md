...
Nous avons maintenant le battement de cœur complet d'un agent : appeler, agir, renvoyer, répondre, recommencer. Mais notre agent n'a toujours qu'un seul outil, câblé en dur dans la boucle. Les vrais agents portent une boîte à outils, et la question intéressante devient celle du *choix* : face à plusieurs outils, comment le modèle choisit-il le bon — ou décide-t-il qu'il n'en a besoin d'aucun ? [Agent-007]({{ site.baseurl }}{% post_url 2026-07-01-agent-007-multi-tool-dispatch-fr %}) donne à notre agent un deuxième outil et un vrai registre pour arbitrer entre eux, transformant le gadget unique codé en dur en un système où n'importe quel outil peut se brancher. La boucle que nous venons de construire est exactement ce qui rend cela possible — la prochaine fois.
